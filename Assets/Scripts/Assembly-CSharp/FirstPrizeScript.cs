...
public class FirstPrizeScript : MonoBehaviour
{
	...
	// Initizialize Variables and start Wandering
	private void Start()
	{
	...
	private void Update()
	{
		...
		if (this.autoBrakeCool > 0f) this.autoBrakeCool -= 1f * Time.deltaTime;
		else this.agent.autoBraking = true;
	...
	// Drive towards player or wander if they're not visible
    ...
    // Select a random hallway and go there (Refer to AILocationSelectorScript)
    private void Wander()
	{
	...
	// Push the player and announce hug
	...
	// Cooldown if the player leaves 1st Prizes Trigger
	private void OnTriggerExit(Collider other)
	{
		if (other.tag == "Player") this.autoBrakeCool = 1f;
	}

	// Called when the Player interacts using Safety Scissors
	public void GoCrazy()
	{
		this.crazyTime = 15f;
	}

...
public class GameOverScript : MonoBehaviour
{
    ...
    // Get a random image or 2% Chance of easter egg that crashes the game (red eyes)
    private void Start()
	{
		...
		else this.image.sprite = this.rare;
	}

	// Handle GameOver Scene Logic
	private void Update()
	{
		this.delay -= 1f * Time.deltaTime;
		if (this.delay <= 0f)
		{
			if (this.chance < 98f) SceneManager.LoadScene("MainMenu");
			else
			{
				...
				if (this.delay <= -5f) Application.Quit();
			}
		}
	}

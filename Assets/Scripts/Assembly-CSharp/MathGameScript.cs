...
public class MathGameScript : MonoBehaviour
{
    ...
    // Start the Learning Game (You can think pad) and hide baldi if spoop mode is enabled
    private void Start()
    {
    ...
    // Handle Baldi Talking and Keyboard input
    private void Update()
    {
    ...
    // Generate a new Math Problem (or an impossible one because of the third problem)
    private void NewProblem()
    {
        ...
        {
            this.endDelay = 5f;
            if (!this.gc.spoopMode) this.questionText.text = "WOW! YOU EXIST!";
    ...
    // Load Easter Eggs, add Anger or just continue
    ...
    // Shift the Queue Left once, overwriting the next audio in queue
    ...
    // Sets AudioInQueue to zero, preventing any Queued audio to play
    private void ClearAudioQueue()
    {
        this.audioInQueue = 0;
    }

    // Exit the You can think pad
    ...
    // Handle the On-Screen button panel
    public void ButtonPress(int value)
    {
        if (value >= 0 & value <= 9) this.playerAnswer.text = this.playerAnswer.text + value;
        else if (value == -1) this.playerAnswer.text = this.playerAnswer.text + "-";
        else this.playerAnswer.text = string.Empty;
    }

...
public class PlaytimeScript : MonoBehaviour
{
	...
	// Handle Cooldown and Disappointed Boolean (Oooh, that makes me sad!)
	private void Update()
	{
		if (this.coolDown > 0f) this.coolDown -= 1f * Time.deltaTime;
		if (this.playCool >= 0f)this.playCool -= Time.deltaTime;
	...
	// Chase after the Player or Wander
	private void FixedUpdate()
	{
		if (!this.ps.jumpRope)
		{
			...
			else if (this.agent.velocity.magnitude <= 1f & this.coolDown <= 0f) this.Wander();
			this.jumpRopeStarted = false;
		}
		else
		{
			// Teleport back and start Jumprope game
			if (!this.jumpRopeStarted) this.agent.Warp(base.transform.position - base.transform.forward * 10f);
    ...
    // Wander to a random Target (Refer to AILocationSelectorScript)
    private void Wander()
	{
	...
	// Don't be depressed anymore and Chase after the player

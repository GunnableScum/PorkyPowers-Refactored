...
public class PrincipalScript : MonoBehaviour
{
	...
	// Chase after the player if they see a rule break for more than >=0.5 seconds
	private void Update()
	{
		...
		else this.timeSeenRuleBreak = 0f;
		if (this.coolDown > 0f) this.coolDown -= 1f * Time.deltaTime;
	}

	// Beeline to the player (or Bully) or wander if they haven't broken a rule
	private void FixedUpdate()
	{
		if (!this.angry)
		{
			...
			{
				this.seesRuleBreak = false;
				if (this.agent.velocity.magnitude <= 1f & this.coolDown <= 0f) this.Wander();
			}
	...
	// Select and wander (Refer to AILocationSelectorScript)
	private void Wander()
	{
		...
		this.coolDown = 1f;
		if (UnityEngine.Random.Range(0f, 10f) <= 1f) this.quietAudioDevice.PlayOneShot(this.aud_Whistle);
	}
	...
	// Scream at the player for breaking Rules
	private void CorrectPlayer()
	{
		this.audioQueue.ClearAudioQueue();
		if (this.playerScript.guiltType == "faculty") this.audioQueue.QueueAudio(this.audNoFaculty);
		else if (this.playerScript.guiltType == "running") this.audioQueue.QueueAudio(this.audNoRunning);
		else if (this.playerScript.guiltType == "drink") this.audioQueue.QueueAudio(this.audNoDrinking);
		else if (this.playerScript.guiltType == "escape") this.audioQueue.QueueAudio(this.audNoEscaping);
	}

	// Update Values and Optionally teleport Player to Detention
	...
	// Change Values depending on what trigger was left
	private void OnTriggerExit(Collider other)
	{
		if (other.name == "Office Trigger") this.inOffice = false;
		if (other.name == "Its a Bully") this.bullySeen = false;
	}
